...
在ELF文件一节中，我们有介绍过ELF文件中常见的一些sections及其作用，包括符号相关的符号表（.symtab）、字符串标（.strtab）。如果考虑动态链接库的话，还要考虑动态符号表（.dynsym）、动态字符串标（.dynstr），它们与.symtab、.strtab的作用是近似的，都是为了解决链接的问题，一个是解决静态链接问题，一个是解决动态链接问题。

我们提到，符号表记录了程序中全局函数、全局变量、局部非静态变量与链接器符号解析、重定位相关的信息。我们还提到，这里提及的符号与我们常提起的调试符号（如gcc -g生成）是没有什么关系的（除了字符串表也包含了.debug_* sections中定义的符号以外）。
