...
有些工程师认为他们不再需要调试器，靠经验和思路就可以高效定位问题。经验确实减少了对调试器的依赖，但这只能说明在其熟悉领域内得心应手，还不能论证调试器无用。所以，我们不能简单地认为“~~**不需要调试器**~~”这个观点。实际上，是否需要调试器，取决于多种因素，如具体问题的性质，不同分析方法的可行性，需要投入的时间精力，以及问题定位效率等。很多时候不是调试器无用，而是在特定问题场景下还没有到必须用调试器来解决问题的地步而已。比如问题场景是，让你调查一个运行时内存分配器创建1GB ballast、mspan脏页导致的内存提交问题呢？
