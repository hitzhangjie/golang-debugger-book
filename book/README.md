...
- 2022~2023年开始，将多年项目踩坑经验整理成电子书，详见 [Go"安全"编程]()，25年开始陆续更新到 [公众号文集](https://mp.weixin.qq.com/mp/appmsgalbum?__biz=MzU2NjY3MTc3MQ==&action=getalbum&album_id=3544414370141700100#wechat_redirect)；
...
有些工程师认为他们不再需要调试器，靠经验和思路就可以高效定位问题。经验确实减少了对调试器的依赖，但这只能说明在其熟悉领域内得心应手，还不能论证调试器无用。所以，我们不能简单地认为“~~**不需要调试器**~~”这个观点。是否需要调试器，取决于多种因素，如具体问题的性质，不同分析方法的可行性，需要投入的时间精力，以及问题定位效率等。很多时候不是调试器无用，而是在特定问题场景下还没有到必须用调试器来解决问题的地步。go+ballast调优GC，这个对gophers肯定不陌生，但未必用过ballast的人都踩过这个坑——ballast初始化时mspan脏页导致内存提交，[预期是ballast只占虚地址空间](https://blog.twitch.tv/en/2019/04/10/go-memory-ballast-how-i-learnt-to-stop-worrying-and-love-the-heap/#:~:text=Now%20onto%202,to%20be%20allocated.)，如果没有调试器协助这个排查过程会复杂许多。
