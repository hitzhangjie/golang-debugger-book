...
| Tracer                     | 泛指调试器对应的进程，严格来讲也是线程，以Linux为例tracer指的是通过ptrace系统调用+PTRACE_ATTACH/PTRACE_SEIZE与tracee建立跟踪、被跟踪关系的调试器中的线程。Linux内核要求ptrace attach之后的后续ptrace请求必须来自发起ptrace attach请求的线程，因此Linux下的调试器实现往往会有一个专门线程作为tracer，负责与被调试进程中的多个tracee进行交互 |
