...
除了跟踪所有线程，dlv调试器还提供切换线程上下文的能力。比如当调试人员希望查看某个特定线程的状态时，可以通过 `dlv>threads` 查看线程列表，然后使用 `dlv> thread <n>` 来切换到特定线程的上下文，这样后续的寄存器查看、变量打印等命令就会显示该线程上下文下的值。另外，dlv也提供了查看goroutines列表并切换goroutine上下文的能力。Go程序天然是多线程程序，而且是提供给开发者的是goroutine并发接口，并不是thread并发相关的接口，所以即使dlv有这个能力，也不一定经常会用到thread相关的调试命令，因为gmp调度模型的存在，你也不确定同一个thread上执行的到底是啥，它执行的goroutine会切换来切换去。反倒是 `dlv> goroutines` 和 `dlv> goroutine <n>` 使用频率更高。

anyway，像C、C++程序需要显示编写多线程代码才会有多线程能力，而Go程序天然就是多线程程序，比如程序启动后就会创建执行sysmon的专用线程，以及调度goroutines的线程，如果goroutine执行逻辑陷入阻塞型系统调用，还有可能会创建新的线程。因此面向Go程序的调试器，必须对多线程这块进行妥善支持，否则压根没法调试。

>ps: 比如我们在两个地址处addr1、addr2分别设置了断点，但是从某个线程命中addr1处断点停下后，我们显示执行continue，再到某个线程执行到addr2处断点停下，我们能简单认为停的是同一个线程吗？不能。甚至我们应该去想，是不是同一个goroutine …… 真实情况比我们想象中要复杂。

篇幅原因，godbg中对多线程调试的调试器支持代码这里不做展示，您可以查看 [hitzhangjie/godbg](https://github.com/hitzhangjie/godbg) 源码进行了解。
